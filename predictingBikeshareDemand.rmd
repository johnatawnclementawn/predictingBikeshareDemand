...
ride.Test <- filter(ride.panel, (week >= 35) & (week <= 36))
...
removeList <- rbind(anti_join(ride.Train, ride.Test, by = "start_station_name"),
                    anti_join(ride.Test, ride.Train, by = "start_station_id")) %>%
...
```{r five_models }
# Temporal Only
reg1 <- lm(Trip_Count ~  hour(interval60) + dotw + Temperature + Precipitation + Wind_Speed, data=ride.Train)
