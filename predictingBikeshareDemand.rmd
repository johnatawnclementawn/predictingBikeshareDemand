...
#%>%
  #st_as_sf(coords = c("start_lng", "start_lat"), crs=crs)
```
...
### 2.1 Link rides with stations
```{r}
startRide_docks <- 
  dplyr::left_join(st_drop_geometry(rides), docks, by =c("start_station_name" = "NAME")) %>%
  st_sf() %>% 
  filter(is.na(OBJECTID) == FALSE)

# endRide_docks <- 
#   dplyr::left_join(st_drop_geometry(rides), docks, by =c("end_station_name" = "NAME")) %>%
#   st_sf() %>% 
#   filter(is.na(OBJECTID) == FALSE)
```
...
ggplot(rides %>%
  ...
  labs(title="Capital Bikeshare trips per hr. Wash. DC, 2021 Sep-Oct",
       x="Date", 
       y="Number of trips") +
  plotTheme()
